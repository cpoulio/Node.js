...
verify() {
    ...
    echo "$DATE - ---------------------- Starting NodeJS System Audit & Candidate Script ----------------------" | tee -a "$LOG_FILE"
    echo "$DATE - REMINDER: Backup your data before deleting any Node.js binaries or symlinks." | tee -a "$LOG_FILE"
    ...
    echo "$DATE - ----- [Node.js Binaries by Location] -----" | tee -a "$LOG_FILE"
    node_found=0
    removal_candidates=0

    while IFS= read -r node_path; do
        if [[ "$node_path" == *"/usr/local/lib/nodejs/"* ]]; then
            echo "$DATE - REMOVAL CANDIDATE: $node_path (version: $("$node_path" -v 2>/dev/null || echo N/A))" | tee -a "$LOG_FILE"
            removal_candidates=$((removal_candidates+1))
        else
            echo "$DATE - DO NOT TOUCH: $node_path (version: $("$node_path" -v 2>/dev/null || echo N/A))" | tee -a "$LOG_FILE"
        fi
        node_found=1
    ...
    if [[ "$node_found" -eq 0 ]]; then
        echo "$DATE - No Node.js binaries found." | tee -a "$LOG_FILE"
    fi

    # ----- [npm Binaries by Location] -----
    echo "$DATE - ----- [npm Binaries by Location] -----" | tee -a "$LOG_FILE"
    npm_found=0
    while IFS= read -r npm_path; do
        if [[ "$npm_path" == *"/usr/local/lib/nodejs/"* ]]; then
            echo "$DATE - REMOVAL CANDIDATE: $npm_path (version: N/A)" | tee -a "$LOG_FILE"
        else
            echo "$DATE - DO NOT TOUCH: $npm_path (version: N/A)" | tee -a "$LOG_FILE"
        fi
        npm_found=1
    ...
    if [[ "$npm_found" -eq 0 ]]; then
        echo "$DATE - No npm binaries found." | tee -a "$LOG_FILE"
    fi

    # ----- [npx Binaries by Location] -----
    echo "$DATE - ----- [npx Binaries by Location] -----" | tee -a "$LOG_FILE"
    npx_found=0
    while IFS= read -r npx_path; do
        if [[ "$npx_path" == *"/usr/local/lib/nodejs/"* ]]; then
            echo "$DATE - REMOVAL CANDIDATE: $npx_path (version: N/A)" | tee -a "$LOG_FILE"
        else
            echo "$DATE - DO NOT TOUCH: $npx_path (version: N/A)" | tee -a "$LOG_FILE"
        fi
        npx_found=1
    ...
    if [[ "$npx_found" -eq 0 ]]; then
        echo "$DATE - No npx binaries found." | tee -a "$LOG_FILE"
    fi

    # ----- [Symlinks in System Bins] -----
    echo "$DATE - ----- [Symlinks in System Bins] -----" | tee -a "$LOG_FILE"
    for bin in node npm npx; do
        for link in /usr/local/bin/$bin; do
            if [ -L "$link" ]; then
                target=$(readlink "$link")
                if [[ "$target" == *"/usr/local/lib/nodejs/"* ]]; then
                    echo "$DATE - REMOVAL CANDIDATE SYMLINK: $link -> $target" | tee -a "$LOG_FILE"
                else
                    echo "$DATE - DO NOT TOUCH SYMLINK: $link -> $target" | tee -a "$LOG_FILE"
                fi
            fi
        done
    done
    ...
    echo "$DATE - ----- [Running Node.js Processes] -----" | tee -a "$LOG_FILE"
    pgrep -a node | while read -r pid cmd; do
        echo "$DATE - NODE PROCESS: $pid $cmd" | tee -a "$LOG_FILE"
    done

    echo "$DATE - ----- [End of Node.js System Audit & Candidate List] -----" | tee -a "$LOG_FILE"
    echo | tee -a "$LOG_FILE"
